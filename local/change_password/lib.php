...
function my_check_password_policy($password, &$errmsg, $data = null)
{
    ...
    if (!empty($CFG->passwordpolicy)) {
        ...
        if (substr($password, 0, 3) == substr(is_object($data) ? $data->username : $data['username'], 0, 3) || substr($password, 0, 3) == mb_substr(is_object($data) ? $data->firstname : $data['firstName'], 0, 3)) {
            $errmsg .= '<div>' . get_string('errormatchpasswordandusername', 'local_change_password', $CFG->minpasswordlength) . '</div>';
        }
        //Проверка на использование верних и нижних регистров
        if (preg_match_all('/[[:digit:]]/u', $password, $matches) < $CFG->minpassworddigits) {
            $errmsg .= '<div>' . get_string('errorminpassworddigits', 'local_change_password', $CFG->minpassworddigits) . '</div>';
        }
        if (preg_match_all('/[[:lower:]]/u', $password, $matches) < $CFG->minpasswordlower) {
            $errmsg .= '<div>' . get_string('errorminpasswordlower', 'local_change_password', $CFG->minpasswordlower) . '</div>';
        }
        if (preg_match_all('/[[:upper:]]/u', $password, $matches) < $CFG->minpasswordupper) {
            $errmsg .= '<div>' . get_string('errorminpasswordupper', 'local_change_password', $CFG->minpasswordupper) . '</div>';
        }
        ...
        if (preg_match_all('/[^[:upper:][:lower:][:digit:]]/u', $password, $matches) < $CFG->minpasswordnonalphanum) {
            $errmsg .= '<div>' . get_string('errorminpasswordnonalphanum', 'local_change_password', $CFG->minpasswordnonalphanum) . '</div>';
        }
        //проверка на одинаковые символы
        if (!check_consecutive_identical_characters($password, $CFG->maxconsecutiveidentchars)) {
            $errmsg .= '<div>' . get_string('errormaxconsecutiveidentchars', 'local_change_password', $CFG->maxconsecutiveidentchars) . '</div>';
        }
    }
...
function core_login_token_update_password($token)
{
    ...
    if (empty($user) or ($user->timerequested < (time() - $pwresettime - DAYSECS))) {
        ...
        echo $OUTPUT->header();
        notice(get_string('noresetrecord', 'local_change_password'), $forgotpasswordurl);
        die; // Never reached.
    }
    if ($user->timerequested < (time() - $pwresettime)) {
        ...
        echo $OUTPUT->header();
        notice(get_string('resetrecordexpired', 'local_change_password', '', $pwresetmins), $forgotpasswordurl);
        die; // Never reached.
    }
    ...
    if ($user->auth === 'nologin' or !is_enabled_auth($user->auth)) {
        ...
        echo $OUTPUT->header();
        print_error('forgotteninvalidurl', 'local_change_password');
        die; // Never reached.
    }

    // Check this isn't guest user.
    if (isguestuser($user)) {
        print_error('cannotresetguestpwd', 'local_change_password');
    }
...
function send_password_to_user_email($user, $resetrecord)
{
    ...
    $message = get_string('emailresetconfirmation', 'local_change_password', $data);
    $subject = get_string('emailresetconfirmationsubject', 'local_change_password', format_string($site->fullname));
